  ...
  return (
    <div className="space-y-6 animate-fade-in pb-24">
      ...
      <div className="flex justify-between items-center">
        <div>
            <h2 className="text-3xl font-serif text-stone-900">Bar</h2>
            <p className="text-stone-500 text-xs">Mixologie & Spiritueux</p>
        </div>
        <button 
      ...
      <div className="flex bg-stone-100 p-1 rounded-xl border border-stone-200">
          <button onClick={() => setActiveTab('STOCK')} className={`flex-1 py-2 text-sm rounded-lg transition-colors ${activeTab === 'STOCK' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-800'}`}>Mon Bar</button>
          <button onClick={() => setActiveTab('COCKTAILS')} className={`flex-1 py-2 text-sm rounded-lg transition-colors ${activeTab === 'COCKTAILS' ? 'bg-white text-stone-900 shadow-sm' : 'text-stone-500 hover:text-stone-800'}`}>Cocktails</button>
      </div>
          ...
          <div className="space-y-4">
               ...
               <div className="relative">
                   ...
                   <input
                       ...
                       className="w-full bg-white border border-stone-200 rounded-xl py-3 pl-10 pr-4 text-stone-900 focus:ring-2 focus:ring-wine-500 outline-none shadow-sm"
                   />
               </div>

               <button 
                onClick={() => setShowAddModal(true)}
                className="w-full py-3 border border-dashed border-stone-400 text-stone-500 rounded-xl hover:bg-stone-100 hover:text-stone-800 transition-colors flex items-center justify-center gap-2 bg-stone-50"
               >
               ...
               <div className="grid gap-4">
                  {filteredSpirits.map(spirit => {
                      ...
                      <div 
                        key={spirit.id} 
                        className="bg-white rounded-xl border border-stone-200 flex hover:border-stone-400 transition-all relative group shadow-sm overflow-hidden"
                      >
                          ...
                          <div className="flex-1 p-4 flex gap-4 items-start">
                              <div className="w-16 h-16 bg-stone-100 rounded-lg flex items-center justify-center border border-stone-200 text-stone-600 relative flex-shrink-0">
                                  <SpiritIcon size={24} />
                                  {spirit.isLuxury && <div className="absolute -top-2 -left-2 bg-purple-100 text-purple-600 p-1 rounded-full border border-purple-200 shadow-sm"><Gem size={10} /></div>}
                              </div>
                              
                              <div className="flex-1 min-w-0">
                                  <div className="flex justify-between items-start mb-2">
                                      <div className="flex-1 min-w-0">
                                          <h3 className="text-lg font-serif text-stone-900 truncate">{spirit.name}</h3>
                                          <p className="text-xs text-stone-500 truncate">{spirit.distillery} • {spirit.age} • {spirit.abv}%</p>
                                      </div>
                                      <span className="text-[10px] bg-stone-100 px-2 py-0.5 rounded text-stone-600 border border-stone-200 ml-2 flex-shrink-0">{spirit.category}</span>
                                  </div>
                                  
                                  {/* Slider Interactif de Niveau */}
                                  <div className="mb-2">
                                      <div className="flex items-center justify-between mb-1">
                                          <span className="text-xs text-stone-500">Niveau restant</span>
                                          <span className={`text-xs font-bold ${
                                              spirit.inventoryLevel > 50 ? 'text-green-600' :
                                              spirit.inventoryLevel >= 20 ? 'text-yellow-600' :
                                              'text-red-600'
                                          }`}>
                                      ...
                                      <div 
                                          className="relative w-full h-3 bg-stone-100 rounded-full overflow-hidden cursor-pointer border border-stone-200 hover:border-stone-400 transition-all group/bar"
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                      {spirit.aromaProfile.slice(0,3).map((a,i) => (
                                          <span key={i} className="text-[9px] bg-stone-50 border border-stone-200 text-stone-600 px-1.5 py-0.5 rounded">{a}</span>
                                      ))}
                                  </div>
                              </div>
          ...
          <div className="space-y-6">
              <form onSubmit={handleSearchCocktails} className="relative">
                  ...
                  <input 
                      ...
                      className="w-full bg-white border border-stone-200 rounded-xl py-2.5 pl-10 pr-4 text-stone-900 focus:ring-2 focus:ring-indigo-500 outline-none"
                  />
              </form>
              
              <button onClick={() => setShowAIChat(true)} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-3 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/20">
                  <MessageSquare size={18} /> Discuter avec le Barman (IA)
              </button>

              {!searchQuery && (
                  <div>
                      <h3 className="text-stone-500 text-xs font-bold uppercase mb-3">Faisable avec mon stock</h3>
                      <div className="grid gap-3">
                          {cocktails.filter(c => canMakeCocktail(c)).map(c => (
                              <div key={c.id} className="bg-white p-4 rounded-xl border border-stone-200 flex items-center gap-4 shadow-sm">
                                  ...
                                  <div>
                                      <h4 className="text-stone-900 font-serif">{c.name}</h4>
                                      <p className="text-xs text-stone-500">{c.ingredients.length} ingrédients</p>
                                  </div>
                                  <div className="ml-auto text-green-600 text-xs flex items-center gap-1">
              ...
              {(apiResults.length > 0 || isSearchingApi) && (
                  <div>
                      <h3 className="text-stone-500 text-xs font-bold uppercase mb-3">Résultats web</h3>
                      {isSearchingApi ? (
                          <div className="flex justify-center py-8"><Loader2 className="animate-spin text-indigo-500"/></div>
                      ) : (
                          <div className="grid gap-3">
                              {apiResults.map(c => (
                                  <div key={c.id} className="bg-white p-4 rounded-xl border border-stone-200 shadow-sm">
                                      <div className="flex items-center justify-between mb-2">
                                          ...
                                          <button onClick={() => handleImportRecipe(c)} className="text-xs bg-stone-100 hover:bg-stone-200 text-stone-900 px-3 py-1.5 rounded-lg transition-colors">
                                              Importer
                                          </button>
                                      </div>
      ...
      {showAddModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
              <div className="absolute inset-0 bg-stone-900/20 backdrop-blur-sm" onClick={() => setShowAddModal(false)} />
              <div className="bg-white border border-stone-200 w-full max-w-sm rounded-2xl p-6 relative z-10 shadow-2xl animate-fade-in-up">
                  <button onClick={() => setShowAddModal(false)} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600"><X size={20} /></button>
                  <h3 className="text-xl font-serif text-stone-900 mb-1">Ajouter au Bar</h3>
                  ...
                  <form onSubmit={handleAddSpirit} className="space-y-4">
                      <div>
                          <label className="text-sm text-stone-500 block mb-2">Nom du Spiritueux</label>
                          <input 
                            autoFocus
                            type="text" 
                            value={newSpiritName}
                            ...
                            className="w-full bg-stone-50 border border-stone-200 rounded-lg p-3 text-stone-900 focus:ring-2 focus:ring-indigo-500 outline-none"
                          />
                      </div>
                      <button 
                        type="submit" 
      ...
      {showAIChat && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
              <div className="absolute inset-0 bg-stone-900/20 backdrop-blur-sm" onClick={() => setShowAIChat(false)} />
              <div className="bg-white border border-stone-200 w-full max-w-md rounded-2xl p-6 relative z-10 shadow-2xl animate-fade-in-up">
                  <button onClick={() => setShowAIChat(false)} className="absolute top-4 right-4 text-stone-400 hover:text-stone-600"><X size={20} /></button>
                  
                  <div className="flex items-center gap-3 mb-4 text-indigo-600">
                      <MessageSquare size={24} />
                      <h3 className="text-xl font-serif text-stone-900">Le Barman Virtuel</h3>
                  </div>
                  ...
                  <textarea 
                    ...
                    className="w-full h-32 bg-stone-50 border border-stone-200 rounded-xl p-4 text-stone-900 focus:ring-2 focus:ring-indigo-500 outline-none resize-none mb-4"
                  />

                  <div className="flex items-center gap-2 text-[10px] text-purple-600 mb-4 bg-purple-50 p-2 rounded-lg border border-purple-200">
                      <Gem size={12} />
      ...
      {showPartyModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
              <div className="absolute inset-0 bg-stone-900/20 backdrop-blur-sm" onClick={() => setShowPartyModal(false)} />
              <div className="bg-white border border-stone-200 w-full max-w-md rounded-2xl p-6 relative z-10 shadow-2xl animate-fade-in-up max-h-[90vh] overflow-y-auto">
                  <div className="flex items-center gap-3 mb-6">
                      <div className="w-10 h-10 rounded-full bg-indigo-600 flex items-center justify-center text-white">
                          <PartyPopper size={20} />
                      </div>
                      <h3 className="text-xl font-serif text-stone-900">Mode Party</h3>
                  </div>

                  <div className="space-y-6">
                      <div>
                          <label className="text-sm text-stone-500">Nombre d'invités</label>
                          <div className="flex items-center gap-4 mt-2">
                              ...
                              />
                              <span className="text-2xl font-bold text-stone-900 w-12 text-center">{guestCount}</span>
                          </div>
                      </div>

                      <div>
                          <label className="text-sm text-stone-500 mb-2 block">Sélectionnez les cocktails</label>
                      ...
                      {Object.keys(partyIngredients).length > 0 && (
                          <div className="bg-stone-50 p-4 rounded-xl border border-stone-200 animate-fade-in">
                              <h4 className="text-stone-900 font-bold mb-3 flex items-center gap-2"><AlertCircle size={16} className="text-indigo-500"/> Ingrédients Nécessaires</h4>
                              <ul className="space-y-2 text-sm text-stone-600">
                                  {Object.entries(partyIngredients).map(([name, amount]) => (
                                      <li key={name} className="flex justify-between border-b border-stone-200 pb-1">
                                          <span>{name}</span>
                                          <span className="text-stone-900 font-mono">{amount}</span>
                                      </li>
                                  ))}
                              </ul>
                          </div>
