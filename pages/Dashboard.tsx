        ...
        <div className="flex items-center gap-2 pt-2">
          ...
          <div className="flex flex-wrap gap-2">
            {wine.suggestedFoodPairings?.slice(0, 3).map((pair, i) => (
              <span key={i} className="text-xs bg-stone-100 dark:bg-stone-800 px-2 py-1 rounded-md text-stone-700 dark:text-stone-300 border border-stone-200 dark:border-stone-700/50">
                {pair}
              </span>
            ))}
          </div>
...
export const Dashboard: React.FC = () => {
  ...
  const loadData = () => setWines(getInventory());
  ...
  const handleConsume = (id: string, e: React.MouseEvent) => {
    ...
    if (target) setConsumingWine(target);
  };
  ...
  const handleCardClick = (id: string) => navigate(`/wine/${id}`);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 w-full max-w-sm rounded-2xl p-6 relative z-10 shadow-2xl animate-fade-in-up">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => setConsumingWine(null)} className="text-stone-400 hover:text-stone-600 dark:hover:text-white">
                <X size={20} />
              </button>
            </div>

            <p className="text-xs text-stone-500 mb-4">
              Sélectionnez la bouteille exacte que vous avez ouverte.
            </p>
