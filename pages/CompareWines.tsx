    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-sm space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {selected.length === 0 && (
            <p className="text-sm text-stone-400">Sélectionnez 2 à 3 vins à comparer</p>
          )}
        </div>
        ...
        {selected.length < 3 && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-stone-50 border border-stone-200 rounded-xl pl-10 pr-4 py-3 text-sm text-stone-900 outline-none focus:ring-2 focus:ring-wine-600"
            />
            {search.trim() && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-stone-200 rounded-xl shadow-xl max-h-48 overflow-y-auto z-20">
                {filtered.slice(0, 8).map(w => (
                  <button
                    key={w.id}
                    onClick={() => addWine(w)}
                    className="w-full text-left px-4 py-3 hover:bg-stone-50 text-sm transition-colors border-b border-stone-100 last:border-0"
                  >
      ...
      {selected.length >= 2 && (
        <div className="bg-white p-6 rounded-2xl border border-stone-200 shadow-sm">
          <h3 className="text-lg font-serif text-stone-800 mb-4">Profil Gustatif Comparé</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-stone-200">
                <th className="text-left p-4 text-stone-500 font-medium text-xs uppercase tracking-wider"></th>
                {selected.map((w, i) => (
...
const Row: React.FC<{ label: string; values: string[] }> = ({ label, values }) => (
  <tr>
    <td className="p-4 text-stone-500 font-medium text-xs uppercase tracking-wider whitespace-nowrap">{label}</td>
    {values.map((v, i) => (
      <td key={i} className="p-4 text-stone-800">{v}</td>
    ))}
  </tr>
);
