...
const KpiTile: React.FC<{ label: string; value: React.ReactNode; sub?: React.ReactNode; chart?: React.ReactNode }> = ({ label, value, sub, chart }) => (
  <div className="rounded-md border border-stone-200 bg-white p-4">
    ...
    <div className="text-3xl font-medium text-stone-900 mt-1.5 leading-none">{value}</div>
...
export const CockpitDashboard: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-12 gap-4">
      ...
      {/* ───── Sommelier hero ───── */}
      <section className="col-span-12 rounded-md bg-cream-100 p-7 ring-1 ring-stone-900/5">
        ...
        <h2 className="serif text-3xl md:text-4xl text-stone-900 leading-tight">
          Que boire <span className="serif-it text-wine-700">ce soir</span> ?
        </h2>
        <p className="text-stone-600 text-sm mt-3 max-w-md">
          Décris le moment, le plat, l'humeur — ou pose la question.
        </p>
        <form
          onSubmit={handleSommelierSubmit}
          className="mt-6 flex items-center gap-2 rounded-md border border-stone-300 bg-white px-3 h-12 focus-within:ring-2 focus-within:ring-wine-600 focus-within:border-wine-600"
        >
      ...
      <section className="col-span-12 lg:col-span-8 rounded-md border border-stone-200 bg-white overflow-hidden">
        ...
        <div className="px-5 pt-4 pb-2">
          <h3 className="serif-it text-xl text-stone-900">Avant qu'ils ne passent leur pic</h3>
        </div>
        {drinkBefore.length > 0 ? (
          <table className="w-full text-sm">
            <thead>
              <tr className="mono text-[10px] tracking-widest text-stone-500 border-b border-stone-200">
            ...
            <tbody>
              {drinkBefore.slice(0, 5).map((a: any) => (
                <tr key={a.wine.id} className="border-b border-stone-100 hover:bg-stone-50">
                  <td className="px-5 py-2.5">
                    <Link to={`/wine/${a.wine.id}`} className="serif-it text-stone-900 hover:text-wine-700">
                      {a.wine.name}
                    </Link>
                  </td>
                  <td className="py-2.5 text-stone-600 text-xs">{a.wine.region}</td>
                  <td className="py-2.5 text-stone-600 text-xs mono">{a.wine.vintage}</td>
                  <td className="py-2.5">
                  ...
                  <td className="px-5 py-2.5 text-right mono text-xs text-stone-600">×{a.wine.inventoryCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <aside className="col-span-12 lg:col-span-4 rounded-md border border-stone-200 bg-white overflow-hidden">
        <header className="px-5 py-3 border-b border-stone-200 bg-stone-50/40">
          <div className="flex items-center gap-2 mono text-[10px] tracking-widest text-stone-600">
            <Sparkles className="w-3 h-3 text-wine-700" />
            HALL OF FAME · TOP {hallOfFame.length}
          </div>
          <h3 className="serif-it text-xl text-stone-900 mt-0.5">Tes coups de cœur</h3>
        </header>
        {hallOfFame.length > 0 ? (
          <ul className="divide-y divide-stone-100">
            {hallOfFame.map((t, i) => (
              <li key={t.wineId} className="px-5 py-3 hover:bg-stone-50">
                <Link to={`/wine/${t.wineId}`} className="block">
                  <div className="flex items-baseline justify-between gap-3">
                    <div className="flex items-baseline gap-2 min-w-0">
                      ...
                      <span className="serif-it text-stone-900 truncate">{t.wine?.name}</span>
                    </div>
                    <span className="mono text-sm text-wine-700 font-medium flex-shrink-0">{t.rating}/5</span>
                  </div>
      ...
      {/* ───── Recent activity ───── */}
      <section className="col-span-12 rounded-md border border-stone-200 bg-white overflow-hidden">
        <header className="px-5 py-3 border-b border-stone-200 flex items-center justify-between bg-stone-50/40">
          <div className="flex items-center gap-2 mono text-[10px] tracking-widest text-stone-600">
        ...
        <div className="px-5 py-2">
          <h3 className="serif-it text-xl text-stone-900 pt-2 pb-2">Dernières entrées</h3>
        </div>
        {journal.length > 0 ? (
