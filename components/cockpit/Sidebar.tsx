...
const ShortcutItem: React.FC<{ to: string; icon: React.FC<any>; label: string; badge?: string | number }> = ({ to, icon: Icon, label, badge }) => (
  <Link
    to={to}
    className="flex items-center gap-2.5 px-2 py-1 text-sm text-stone-600 hover:text-stone-900"
  >
...
export const Sidebar: React.FC<SidebarProps> = ({ totalWines, insightsCount, wishlistCount }) => {
  ...
  return (
    <aside className="w-[220px] flex-shrink-0 border-r border-stone-200 bg-white py-5 px-3 flex flex-col min-h-[calc(100vh-2.25rem)]">
      ...
      <div className="px-2 pt-5 mt-5 border-t border-stone-200">
        <div className="flex items-center gap-3 mb-4">
          <Link to="/settings" className="text-stone-500 hover:text-stone-900" title="Paramètres" aria-label="Paramètres">
        ...
        <div className="serif-it text-stone-700 text-sm leading-snug">
